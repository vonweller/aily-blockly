...
export function withNativeModelRegistrations<T>(realm: any, generator: Blockly.Generator,
  register: (block: Blockly.Block, name: string, type: string) => void, action: () => T): T {
  ...
  realm.registerVariableToBlockly = helper;
  ...
  for (const [type, callback] of functions) {
    const scoped: Blockly.Generator['forBlock'][string] = function(block, current) {
      ...
      try { return callback.call(this, block, current); } finally { owner = previous; }
    };
  ...
  try {
    ...
    if (realm.registerVariableToBlockly !== helper) throw new Error('Native generator replaced the model registration boundary.');
    ...
    realm.registerVariableToBlockly = original;
