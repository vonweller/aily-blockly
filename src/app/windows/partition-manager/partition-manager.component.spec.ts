...
describe('Partition manager window interactions', () => {
  ...
  // Use the modal's own element; a separately resolved CDK package may expose
  // a different OverlayContainer token in pnpm workspaces.
  const overlay = (): HTMLElement => modal.openModals[0].getElement();
  ...
  it('opens one discard dialog for repeated requests and treats Escape as cancellation', async () => {
    ...
    expect(modal.openModals.length).toBe(1);
