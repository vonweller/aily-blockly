...
export class NodeSerialPortAdapter {
  ...
  // 沿用 ffs-manager 的短暂停放机制；再次 open 前先真正关闭旧句柄。
  // 读取结束时 dispose() 会立即释放，不等待停放计时器。
  ...
  async open(options: NodeSerialOpenOptions): Promise<void> {
    if (this.disposed) throw new Error('串口读取已结束');
    if (this.port) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (this.disposed) {
          await this.forceCloseRaw(candidate);
          throw new Error('串口读取已结束');
        }
   ...
   * 如果在停放窗口内 open() 被再次调用，则先关闭旧句柄再重建连接。
   * 否则停放超时后关闭；dispose() 始终立即释放。
   */
  ...
  private async forceCloseRaw(port: RawSerialPort): Promise<void> {
    await new Promise<void>((resolve) => {
      try {
        // Always call the proxied close method; a bridged isOpen value may be stale.
        port.close((err: Error | null | undefined) => {
          if (err && !/not open/i.test(err.message || '')) console.warn('[NodeSerialPortAdapter] forceCloseRaw 失败:', err);
          resolve();
        });
      } catch (err) {
  ...
  dispose(): Promise<void> {
    if (!this.disposePromise) {
      this.disposed = true;
      this.disposePromise = this.disposePort();
    }
    return this.disposePromise;
  }

  private async disposePort(): Promise<void> {
    await this.close();
    if (this.parkTimer) { clearTimeout(this.parkTimer); this.parkTimer = null; }
    const parked = this.parkedPort;
    this.parkedPort = null;
