...
@Component({
  ...
  template: `
    <div class="aily-state-viewer" [ngClass]="getStateClass()">
      <div class="state-content">
        <div class="state-icon" *ngIf="getStateIcon()">
          <span nz-icon [nzType]="getStateIcon()" [nzSpin]="isSpinning()"></span>
        </div>
        <div class="state-text">
          {{ getDisplayText() }}
        </div>
        <div class="state-progress" *ngIf="stateInfo?.progress !== undefined">
          <nz-progress 
            [nzPercent]="stateInfo.progress" 
            [nzSize]="'small'"
            [nzShowInfo]="false">
          </nz-progress>
        </div>
  ...
  styles: [`
    ...
    .aily-state-viewer.loading {
      background-color: #e6f7ff;
      border-left-color: #1890ff;
    }

    .aily-state-viewer.success {
      background-color: #f6ffed;
      border-left-color: #52c41a;
    }

    .aily-state-viewer.error {
      background-color: #fff2f0;
      border-left-color: #ff4d4f;
    }

    .aily-state-viewer.warning {
      background-color: #fffbe6;
      border-left-color: #faad14;
    }

    .aily-state-viewer.info {
