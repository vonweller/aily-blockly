  ...
  template: `
    <div class="cca-actions">
      <div class="cca-split-btn">
        ...
        >{{ primaryLabel }}</button>
        @if (hasMoreActions) {
          <button type="button" class="cca-btn-caret" [title]="moreActionsTooltip" (click)="toggleDropdown($event)">
      ...
      <button type="button" class="cca-btn-reject" [title]="rejectTooltip" (click)="reject.emit()">{{ rejectLabel }}</button>
    </div>
  `,
  styles: [`
    :host {
