...
import { buildRunSubagentDescription, buildRunSubagentSchema, registerSubagent, getRegisteredSubagents, getSubagentDefinition } from './runSubagentTool';
...
- uninstall — 卸载已安装的技能`,
    ...
    */
    ...
    // =============================================================================
    {
        ...
        get input_schema() { return buildRunSubagentSchema().input_schema; },
        agents: ["mainAgent"]
    },
    {
        name: 'register_agent',
        description: '动态注册一个新的子代理（subagent），注册后即可通过 run_subagent 调用。用于在运行时创建专用子代理来处理特定领域的任务。已注册的同名代理不会被覆盖。',
        input_schema: {
            type: 'object',
            properties: {
