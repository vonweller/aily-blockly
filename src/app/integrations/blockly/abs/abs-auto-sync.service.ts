...
export class AbsAutoSyncService {
  ...
  /**
   * Applies caller-supplied ABS through the product-owned Blockly workspace.
  ...
  async importContent(absContent: string): Promise<{
    ...
    if (typeof absContent !== 'string' || !absContent.trim()) {
      return { success: false, errors: ['ABS 内容为空。'] };
    }
    ...
    if (!workspace) {
      this.isSyncing = false;
      return { success: false, errors: ['Blockly 工作区未就绪。'] };
    }
    ...
    try {
      ...
      const errors = conversion.errors?.map((error) => `第 ${error.line} 行: ${error.message}`) ?? [];
      const warnings = conversion.warnings?.map((warning) => `第 ${warning.line} 行: ${warning.message}`) ?? [];
      if (!conversion.success || !conversion.abiJson || errors.length > 0) {
      ...
      try {
        workspace.clear();
        Blockly.serialization.workspaces.load(rollbackSnapshot, workspace);
      } finally {
        Blockly.Events.enable();
      }
      workspace.render();
