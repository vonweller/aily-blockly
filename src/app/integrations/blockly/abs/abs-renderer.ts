...
import { AbsAbiBlock, AbsAbiWorkspace, AbsProjectionContracts, AbsSyncError, ABS_SCHEMA_HEADER,
  absFieldPath, absInputPath, getAbsFieldDefinition, isAbsBlockDisabled } from './abs-state';
...
export function renderAbs(workspace: AbsAbiWorkspace, contracts: AbsProjectionContracts, symbols: AbsSymbols, previousProjectionValidation = false) {
  ...
  const call = (block: AbsAbiBlock, path: string): string => {
    ...
    if (!positional) for (const name of Object.keys(block.inputs ?? {})) {
      if (inlineInput(block, name)) args.push(`${identifier(name)}=${inputArgument(name)}`);
    }
  ...
  const render = (block: AbsAbiBlock, path: string, depth: number, chain = false) => {
    ...
    for (const name of Object.keys(block.inputs ?? {})) {
      ...
      const implicit = child && statements?.length === 1 && statements[0].name === name;
