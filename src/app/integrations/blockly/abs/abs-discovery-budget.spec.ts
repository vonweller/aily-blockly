...
describe('board/library discovery budgets', () => {
  ...
  it('restores the original default while respecting an explicit smaller limit', async () => {
    const result = await searchBoardsLibrariesTool.handler({ query: 'sensor', type: 'libraries' }, config);
    expect(result.is_error).toBeFalse();
    expect(result.metadata).toEqual(jasmine.objectContaining({ totalMatches: 17, returnedCount: 17, truncated: false }));
    expect(result.metadata!.results!.length).toBe(17);
