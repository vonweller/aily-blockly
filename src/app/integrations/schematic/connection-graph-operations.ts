...
export interface ConnectionGraphInvocationContext {
  turnId?: string;
  toolCallId?: string;
  recordMutationReceipt?: (receipt: WorkspaceMutationReceipt) => void;
...
function joinHostPath(...parts: string[]): string {
    const pathApi = typeof window !== 'undefined' ? (window as any).path : undefined;
...
function resolveProjectPath(projectService: ProjectService): string | null {
    ...
    for (const candidate of candidates) {
        try {
            if (window['fs'].existsSync(joinHostPath(candidate, 'package.json'))) {
                return candidate;
            }
        } catch {
...
async function resolveBoardPackagePath(projectService: ProjectService): Promise<string | null> {
    ...
    try {
        if (window['fs'].existsSync(packageJsonPath)) {
            packageJson = JSON.parse(window['fs'].readFileSync(packageJsonPath, 'utf8'));
        }
    } catch {
        packageJson = null;
    }
...
这些组件无法生成连线，请先调用 get_project_context 确认其状态，再使用 generate_pinmap + save_pinmap 生成配置。`;
    ...
    const toolResult: HostToolResult = {
      is_error: false,
...
async function trySyncPinmapComponentsFromApi(
  ...
  try {
    ...
    const rawToken = await (invocationContext?.getAuthToken?.() ?? Promise.resolve(null));
    const token =
...
export async function getProjectContextTool(
  ...
  try {
    ...
    let cppCode = '';
    try {
      cppCode = invocationContext?.getGeneratedCode?.() || '';
    } catch { /* ignore */ }
    ...
    let catalogData: any = {};
    try {
      catalogData = JSON.parse(String(catalogResult.content));
    } catch {
      return {
        is_error: true,
