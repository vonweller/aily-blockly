...
> Coder 路径更新（2026-08-20）：下文的 `.temp/*.json` 表示 Blockly 路径；Coder 对应文件位于持久化 `sketch/*.json`。`upload.js` 会按根 `project.aci` 自动选择 `sketch/preprocess.json`，应用层也会把 Coder 的 upload/network OTA 配置写入 `sketch/`。
