...
!macro customInstall
  ...
  ; 自动查找 xpm-*.7z 压缩包并解压到 xpm 目录
  FindFirst $0 $1 "$INSTDIR\resources\child\xpm-*.7z"
  ${If} $1 != ""
    nsExec::ExecToStack '"$INSTDIR\resources\child\7za.exe" x "$INSTDIR\resources\child\$1" -o"$INSTDIR\resources\child\xpm" -y'
