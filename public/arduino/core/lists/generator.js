Arduino.forBlock["lists_create_empty"] = function (block) {
...
Arduino.forBlock["lists_repeat"] = function (block) {
...
Arduino.forBlock["lists_isEmpty"] = function (block) {
...
Arduino.forBlock["lists_indexOf"] = function (block) {
  // Find an item in the list.
  const operator =
    block.getFieldValue("END") === "FIRST" ? "indexOf" : "lastIndexOf";
  const item = Arduino.valueToCode(block, "FIND", Arduino.ORDER_NONE) || "''";
  const list =
    Arduino.valueToCode(block, "VALUE", Arduino.ORDER_MEMBER) || "[]";
...
Arduino.forBlock["lists_getIndex"] = function (block) {
...
Arduino.forBlock["lists_setIndex"] = function (block) {
  // Set element at index.
  // Note: Until February 2013 this block did not have MODE or WHERE inputs.
  let list = Arduino.valueToCode(block, "LIST", Arduino.ORDER_MEMBER) || "[]";
  const mode = block.getFieldValue("MODE") || "GET";
  const where = block.getFieldValue("WHERE") || "FROM_START";
  const value =
    Arduino.valueToCode(block, "TO", Arduino.ORDER_ASSIGNMENT) || "null";
  ...
  function cacheList() {
    ...
    const listVar = Arduino.nameDB_.getDistinctName(
      "tmpList",
      NameType.VARIABLE,
    );
  ...
  switch (where) {
    ...
    case "RANDOM": {
      let code = cacheList();
      const xVar = Arduino.nameDB_.getDistinctName("tmpX", NameType.VARIABLE);
      code +=
...
Arduino.forBlock["lists_getSublist"] = function (block) {
  ...
  const list = Arduino.valueToCode(block, "LIST", Arduino.ORDER_MEMBER) || "[]";
...
Arduino.forBlock["lists_sort"] = function (block) {
  // Block for sorting a list.
  const list =
    Arduino.valueToCode(block, "LIST", Arduino.ORDER_FUNCTION_CALL) || "[]";
  const direction = block.getFieldValue("DIRECTION") === "1" ? 1 : -1;
  const type = block.getFieldValue("TYPE");
  const getCompareFunctionName = Arduino.provideFunction_(
    "listsGetSortCompare",
    `
...
Arduino.forBlock["lists_split"] = function (block) {
...
Arduino.forBlock["lists_reverse"] = function (block) {
